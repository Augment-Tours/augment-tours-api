...
router.get('/:museumId/:floor', async (req, res) => {
  try {
    const armodels = await Armodel.getArmodelsbyMuseumFloor(
      req.params.museumId,
      req.params.floor
    );
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const armodel = await Armodel.addArmodel(
      newArmodel.name,
      newArmodel.description,
      newArmodel.model,
...
router.post('/:id/edit', async (req, res) => {
  console.log('in edit ar model');
  try {
    const newArmodel = req.body;
    console.log('ArModel', newArmodel);
    const armodel = await Armodel.updateArmodels(
      req.params.id,
      newArmodel.name,
      newArmodel.description,
      newArmodel.model,
